...
png(filename = "plot4.png", width = 480, height = 480, units = "px")
        ...
        ## Chart 2 (Top Right)
        with(p1_dfSample, plot(Time, Voltage, type = "n"
                               , xlab = "datetime"
                               , ylab = "Voltage")
             )
        with(p1_dfSample, lines(Time, Voltage))
        ...
        with(p1_dfSample, plot(Time, Global_reactive_power, type = "n"
                               , xlab = "datetime")
             )
        with(p1_dfSample, lines(Time, Global_reactive_power))
